...
def get_loss_and_accuracy(logits, targets, eq_positions, mask, reduction='mean'):
    ...
    # Create masks for the right-hand side of each equation
    # First create a position tensor with values [0, 1, 2, ..., seq_len-1] for each sample in batch
    positions = torch.arange(seq_len, device=logits.device).expand(batch_size, seq_len)
        
    # Create RHS mask: positions > eq_positions and valid according to the mask
    # eq_positions needs to be reshaped to allow broadcasting
    eq_positions_expanded = eq_positions.unsqueeze(1)
    rhs_mask = (positions > eq_positions_expanded) & (mask.bool())
        
    # Get the predicted token indices
    pred_tokens = torch.argmax(logits, dim=-1)
        
    # Calculate accuracy per sequence (1 if all predictions in RHS are correct, 0 otherwise)
    correct_predictions = (pred_tokens == targets) & rhs_mask
      
    # For each sequence, count if all RHS tokens are predicted correctly
    seq_correct = torch.all(correct_predictions | ~rhs_mask, dim=1).float()
        
    # Count number of valid tokens in RHS for each sequence
    rhs_token_counts = rhs_mask.sum(dim=1).float()
        
    # Calculate negative log likelihood for valid RHS tokens
    # Gather the log probabilities of the target tokens
    gather_indices = targets.unsqueeze(-1)
    target_log_probs = torch.gather(log_probs, dim=-1, index=gather_indices).squeeze(-1)
        
        # Apply RHS mask to the target log probabilities and sum for each sequence
    masked_log_probs = target_log_probs * rhs_mask.float()
    seq_loss = -masked_log_probs.sum(dim=1)
        
        # Normalize the loss by the number of valid RHS tokens in each sequence
        # Avoid division by zero by adding a small epsilon
    seq_loss = seq_loss / (rhs_token_counts + 1e-10)
      
        # Apply the requested reduction
    if reduction == 'none':
        loss = seq_loss
        accuracy = seq_correct
    elif reduction == 'sum':
        loss = seq_loss.sum()
        accuracy = seq_correct.sum()
    else:  # 'mean'
        loss = seq_loss.mean()
        accuracy = seq_correct.mean()
