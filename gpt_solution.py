...
class LayerNorm(nn.Module):
    ...
    def forward(self, inputs):
        ...
        """
        # find mean of the last dimensin
        mean = inputs.mean(dim=-1, keepdim=True)

        mean_x2 = (inputs**2).mean(dim=-1, keepdim=True)
        
        var = mean_x2 - mean**2
        
        # normalize
        inputs_norm = (inputs - mean) / torch.sqrt(var + self.eps)

        # scale and shift 
        outputs = inputs_norm * self.weight + self.bias

        return outputs
        # find variance of the last dimension
...
class MultiHeadedAttention(nn.Module):
    ...
    def get_attention_weights(self, queries, keys):
        ...
        # Create causal mask to ensure each position only attends to previous positions
        # Shape: (sequence_length, sequence_length)
        sequence_length = queries.size(2)
        causal_mask = torch.triu(torch.ones(sequence_length, sequence_length, 
                                            device=queries.device), diagonal=1).bool()
        
        # Apply the causal mask to the attention scores
        # Set attention scores for future tokens to negative infinity
    ...
    def apply_attention(self, queries, keys, values):
        ...
        # Shape: (batch_size, num_heads, sequence_length, head_size)
    ...
    def split_heads(self, tensor):
        ...
        """
        batch_size, sequence_length, contact_dim = tensor.size()
        dim = contact_dim // self.num_heads

        tensor = tensor.view(batch_size, sequence_length, self.num_heads, dim)
    ...
    def merge_heads(self, tensor):
        ...
        # Transpose to put sequence_length before num_heads
        tensor = tensor.transpose(1, 2)
        
        # Merge the num_heads and dim dimensions
        output = tensor.contiguous().view(batch_size, sequence_length, num_heads * dim)
    ...
    def forward(self,  queries: Tensor, keys: Tensor, values: Tensor):
        ...
        # Apply attention
        # context: (batch_size, sequence_length, num_heads * head_size)
        ...
        # Since we don't need to backpropagate through them
...
class GPTEmbedding(nn.Module):
    ...
    def create_sinusoidal_embeddings(self, n_positions: int, dimension: int) -> torch.FloatTensor:
        ...
        # Create dimension indices (div by 2 because we use sin/cos pairs)
        div_term = torch.arange(0, dimension, 2, dtype=torch.float)
        # Apply the formula 10000^(2i/d)
        div_term = torch.exp(-math.log(10000.0) * div_term / dimension)
        ...
        # Fill odd indices with cos (if dimension is odd, the last column may be unfilled)
        if dimension % 2 == 0:
            position_enc[:, 1::2] = torch.cos(position * div_term)
        else:
            position_enc[:, 1::2] = torch.cos(position * div_term[:dimension//2])
    ...
    def forward(self, tokens: Tensor) -> Tensor:
        ...
        # Get the position encodings from the pre-computed buffer
        # Shape: (sequence_length, embedding_size)
        position_encodings = self.position_encoding[:sequence_length, :]
...
class GPT(nn.Module):
    ...
    # Get embeddings
        ...
        # decoder_output: (batch_size, sequence_length, embedding_size)
        # decoder_states: tuple of tensors containing hidden states and attention weights
        decoder_output, decoder_states = self.decoder(embeddings)
        
        # Apply classifier to get logits
        # Shape: (batch_size, sequence_length, vocabulary_size)
        logits = self.classifier(decoder_output)
        
        # Return the logits and the decoder states tuple
