...
class LSTMCell(nn.Module):
    ...
    def forward(self, x, hx):
        ...
        c_tilde = torch.tanh(self.candidate_cell(combined))  # candidate cell

        # Compute new updated cell state
        c_t = f * c + i * c_tilde  # (batch_size, hidden_size)
...
class LSTM(nn.Module):
    ...
    def forward(self, x, hx=None):
        ...
        if hx is None:
            h0 = torch.zeros(self.num_layers, batch_size, self.hidden_size, dtype = x.dtype, device = x.device)  # (num_layers, batch_size, hidden_size)
            c0 = torch.zeros(self.num_layers, batch_size, self.hidden_size, dtype = x.dtype, device = x.device)  # (num_layers, batch_size, hidden_size)
        else:
...
class LSTMLM(nn.Module):
    ...
    def forward(self, x, hidden_states=None):
        ...
        # Embedding
        x_embedded = self.embedding(x)  # (batch_size, sequence_length, embedding_size)  

        # LSTM
        lstm_output, hidden = self.lstm(x_embedded, hidden_states)  # (batch_size, sequence_length, hidden_size), (num_layers, batch_size, hidden_size)
